...
import { proxyGrpc } from "../controllers/proxyGrpc";
...
it("unary w/ gRPC server reflection", async () => {
    ...
    const elizaResponse = response.body as ElizaResponse;
...
it.skip("unary w/ default schema", async () => {
    ...
    const upsertResponse = JSON.parse(response.body as string) as UpsertResponse;
...
it("invalid schema", async () => {
    ...
    expect(response).not.toBe(null);
    expect(response.body).not.toBe(null);
    expect(response.body).toEqual('Failure: failed to find service named "connectrpc.eliza.v1.ElizaService" in schema');
