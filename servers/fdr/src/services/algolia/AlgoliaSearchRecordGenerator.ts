...
export class AlgoliaSearchRecordGenerator {
  public constructor(
    protected readonly config: AlgoliaSearchRecordGeneratorConfig
  ) {}
  ...
  protected generateAlgoliaSearchRecordsForUnversionedNavigationConfig(
    ...
    return visitUnversionedDbNavigationConfig(config, {
      tabbed: (tabbedConfig) => {
        return this.generateAlgoliaSearchRecordsForUnversionedTabbedNavigationConfig(
          tabbedConfig,
          context
        );
      },
      untabbed: (untabbedConfig) => {
        return this.generateAlgoliaSearchRecordsForUnversionedUntabbedNavigationConfig(
          untabbedConfig,
          context
        );
      },
  ...
  protected generateAlgoliaSearchRecordsForUnversionedUntabbedNavigationConfig(
    ...
    const records = config.items.map((item) =>
        ...
        ...parents
          ...
          .filter((parent) =>
            parent.type === "apiReference"
              ...
              : parent.type === "changelogMonth" ||
                  parent.type === "changelogYear"
                ? false
                : true
          )
      ...
      if (FernNavigation.V1.isApiLeaf(node)) {
        visitDiscriminatedUnion(node)._visit({
          endpoint: (node) => {
            ...
            contents.push(
              this.collectReferencedTypesToContent(
                typeReferences,
                holder?.api.types ?? {}
              )
            );
            ...
            if (ws.messages.length > 0) {
              ...
              ws.messages.forEach((message) => {
                ...
                if (message.body.type === "reference") {
                  ...
                  contents.push(
                    `- ${this.stringifyTypeRef(message.body.value)}`
                  );
  ...
  protected stringifyTypeRef(typeRef: APIV1Read.TypeReference): string {
    return visitDiscriminatedUnion(typeRef)._visit({
      ...
      map: (value) =>
        `Map<${this.stringifyTypeRef(value.keyType)}, ${this.stringifyTypeRef(value.valueType)}>`,
      unknown: () => "unknown",
    });
  }
      ...
      Object.entries(referencedTypes).forEach(([key, value]) => {
        ...
        visitDiscriminatedUnion(value.shape)._visit({
          ...
          discriminatedUnion: (value) => {
            value.variants.forEach((variant) => {
              contents.push(
                `#### ${variant.displayName ?? titleCase(variant.discriminantValue)}\n`
              );
  ...
  protected generateAlgoliaSearchRecordsForChangelogSection(
    ...
    if (changelog.pageId != null) {
      ...
      if (changelogPageContent != null) {
        const processedContent = convertMarkdownToText(
  ...
  protected generateAlgoliaSearchRecordsForApiDefinition(
    ...
    const subpackagePathParts = getPathPartsBySubpackage({
      definition: apiDef,
    });
    ...
    rootPackage.endpoints.forEach((e) => {
      const endpointRecords =
        this.generateAlgoliaSearchRecordsForEndpointDefinition(e, context);
    ...
    Object.entries(subpackages).forEach(([id, subpackage]) => {
      ...
      if (pathParts == null) {
        LOGGER.error(
          "Excluding subpackage from search. Failed to find path parts for subpackage id=",
          id
        );
        return;
      }
      subpackage.endpoints.forEach((e) => {
        const endpointRecords =
          this.generateAlgoliaSearchRecordsForEndpointDefinition(
            e,
