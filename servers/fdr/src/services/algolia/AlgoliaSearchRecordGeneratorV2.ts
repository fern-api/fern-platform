...
import {
  ...
  getFrontmatter,
} from "./AlgoliaSearchRecordGenerator";
...
export class AlgoliaSearchRecordGeneratorV2 extends AlgoliaSearchRecordGenerator {
  ...
  protected parseMarkdownItem(
    ...
    const markdownSectionRecords = getMarkdownSections(
      markdownTree,
      breadcrumbs,
      indexSegment.id,
      fdrSlug
    ...
    markdownSectionRecords.push(
      compact({
        ...
        description: truncateToBytes(markdownTree.content, 50 * 1000),
        breadcrumbs,
        slug: fdrSlug,
        version,
  ...
  protected generateAlgoliaSectionRecordsForPageNavigationItem(
    ...
    const markdownSectionRecords: AlgoliaSearchRecord[] =
      this.parseMarkdownItem(
  ...
  protected override generateAlgoliaSearchRecordsForNavigationItem(
    ...
    switch (item.type) {
      case "section":
        return this.generateAlgoliaSearchRecordsForSectionNavigationItem(
          item,
          context
        );
      case "api":
        return this.generateAlgoliaSectionRecordsForApiNavigationItem(
          item,
          context
        );
      case "page":
        return this.generateAlgoliaSectionRecordsForPageNavigationItem(
          item,
          context
        );
      case "link":
  ...
  protected addEndpointFieldAndMaybeTypeReferenceToAlgoliaSearchRecords(
    ...
    const breadcrumbs = parentBreadcrumbs.concat(
  ...
  protected generateAlgoliaSearchRecordsForApiReferenceNodeV2(
    ...
    if (api == null) {
      LOGGER.error(
        "Failed to find API definition for API reference node. id=",
        root.apiDefinitionId
      );
    }
    const holder =
      api != null
        ? FernNavigation.ApiDefinitionHolder.create(
            convertDbAPIDefinitionToRead(api)
          )
        : undefined;
    const records: AlgoliaSearchRecord[] = [];
    ...
    const version =
      context.indexSegment.type === "versioned"
        ? ({
            ...
            slug: FernNavigation.V1.Slug(
              context.indexSegment.version.urlSlug ??
                context.indexSegment.version.id
            ),
      ...
      if (FernNavigation.V1.isApiLeaf(node)) {
        ...
        visitDiscriminatedUnion(node)._visit({
          endpoint: (node) => {
            ...
            if (endpoint == null) {
              LOGGER.error(
                "Failed to find endpoint for API reference node.",
                node
              );
              return;
            }
          ...
          webSocket: (node) => {
            ...
            if (ws == null) {
              LOGGER.error(
                "Failed to find websocket for API reference node.",
                node
              );
              return;
            }
          ...
          webhook: (node) => {
            ...
            if (webhook == null) {
              LOGGER.error(
                "Failed to find webhook for API reference node.",
                node
              );
              return;
            }
            ...
            const typeReferences: TypeReferenceWithMetadata[] = [];
            const fields: AlgoliaSearchRecord[] = [];
            const endpointPath: EndpointPathPart[] = webhook.path.map(
              (path) => ({
                type: "literal",
                value: path,
              })
  ...
  private collectReferencedTypeReferenceToContentV2(
    ...
    object.properties.forEach((property) => {
      const slug = FernNavigation.V1.Slug(
        `${baseSlug}.${encodeURI(property.key)}`
      );
    ...
    undiscriminatedUnion.variants.forEach((variant, idx) => {
      const title =
        ...
        (variant.type.type === "id"
          ? titleCase(types[variant.type.value]?.name ?? "")
          : "");
      if (!undiscriminatedUnionVariants.has(title) || title == "") {
        newVariantSet.add(title);
        const slug =
          title != ""
            ? FernNavigation.V1.Slug(`${baseSlug}.${encodeURI(title)}`)
            : baseSlug;
        const anchorIdParts =
          title != ""
  ...
  private collectReferencedDiscriminatedUnionToContentV2(
    ...
    discriminatedUnion.variants.forEach((variant) => {
      if (!discriminatedUnionVariants.has(variant.discriminantValue)) {
        ...
        const title =
          variant.discriminantValue ?? encodeURI(variant.displayName ?? "");
  ...
  protected generateAlgoliaSearchRecordsForChangelogSectionV2(
    ...
    if (changelog.pageId != null) {
      const changelogPageContent =
        this.config.docsDefinition.pages[changelog.pageId];
      ...
      changelog.items.forEach((changelogItem) => {
        ...
        const slug = FernNavigation.V1.Slug(changelog.urlSlug);
        const changelogPageContent =
          this.config.docsDefinition.pages[changelogItem.pageId];
        if (changelogPageContent != null) {
          const markdownSectionRecords = this.parseMarkdownItem(
            changelogPageContent.markdown,
            [],
...
function anchorIdToSlug(
  ...
  return FernNavigation.V1.Slug(
    `${node.slug}#${encodeURI(anchorIdParts.join("."))}`
  );
}

export function getMarkdownSectionTree(
  markdown: string,
  pageTitle: string
): MarkdownNode {
  ...
  const root: MarkdownNode = {
    level: 0,
    heading: frontmatter.title ?? pageTitle,
    content: "",
    children: [],
  };
...
export function getMarkdownSections(
  ...
  const markdownSlug = FernNavigation.V1.Slug(
    markdownSection.level === 0
      ? slug
      : `${slug}#${encodeURI(kebabCase(markdownSection.heading.toLowerCase()))}`
  );
      ...
      : [
          compact({
            ...
            content: truncateToBytes(
              sanitizeText(markdownSection.content.trim()),
              50 * 1000
            ),
  ...
  return records.concat(
    markdownSection.children.reduce(
      (acc: AlgoliaSearchRecord[], markdownSectionChild: MarkdownNode) => {
        return acc.concat(
          getMarkdownSections(
            markdownSectionChild,
            sectionBreadcrumbs,
            indexSegmentId,
            slug
          )
