...
export function convertGeneratorReleaseType(
...
export function convertPrismaReleaseType(
  releaseType: prisma.ReleaseType
): ReleaseType {
  switch (releaseType) {
...
export function getPrereleaseTypeAndVersion(
  version: string
): [ReleaseType, number] {
  ...
  if (
    ...
    parsedVersion.prerelease.length < 3
  ) {
    ...
    const prereleaseTypeWithVersion = prereleaseType.match(
      /((?:rc|alpha|beta))([0-9]+)/
    );
    if (
      ...
      if (parsedVersion.prerelease.length > 1) {
        const truePrereleaseVersion = parseInt(
          parsedVersion.prerelease[1] as string
        );
