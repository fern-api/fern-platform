...
export class CliVersionsDaoImpl implements CliVersionsDao {
  ...
  async getLatestCliRelease({
    ...
    const releaseTypes =
      getLatestCliReleaseRequest.releaseTypes != null
        ? getLatestCliReleaseRequest.releaseTypes.map(
            convertGeneratorReleaseType
          )
  ...
  async upsertCliRelease({
    ...
    const data = {
      ...
      releaseType: convertGeneratorReleaseType(
        getPrereleaseType(cliRelease.version)
      ),
      changelogEntry:
        cliRelease.changelogEntry != null
          ? writeBuffer(cliRelease.changelogEntry)
          : null,
      isYanked:
        cliRelease.isYanked != null ? writeBuffer(cliRelease.isYanked) : null,
      createdAt:
        cliRelease.createdAt != null
          ? new Date(cliRelease.createdAt)
          : undefined,
  ...
  async listCliReleases({
    ...
    return {
      cliReleases: releases
        .map(convertPrismaCliRelease)
        .filter((g): g is CliRelease => g != null),
    };
  }
}

function convertPrismaCliRelease(
  cliRelease: prisma.CliRelease | null
  ...
  return {
    ...
    changelogEntry:
      cliRelease.changelogEntry != null
        ? (readBuffer(cliRelease.changelogEntry) as ChangelogEntry[])
        : undefined,
    majorVersion: cliRelease.major,
    isYanked:
      cliRelease.isYanked != null
        ? (readBuffer(cliRelease.isYanked) as Yank)
        : undefined,
