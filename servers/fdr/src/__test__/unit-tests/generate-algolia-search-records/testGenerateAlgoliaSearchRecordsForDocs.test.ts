    ...
    (only ? it.only : it)(
      ...
      async () => {
        ...
        const preloadApiDefinitions = () => {
          const apiIdDefinitionTuples = docsDefinition.referencedApis.map(
            (id) => {
              const apiDef = loadApiDefinition(fixture, id);
              return [
                id,
                convertAPIDefinitionToDb(apiDef, id, EMPTY_SNIPPET_HOLDER),
              ] as const;
            }
          );
        ...
        const generator = new AlgoliaSearchRecordGeneratorV2({
          docsDefinition,
          apiDefinitionsById,
        });
        ...
        visitDbNavigationConfig(navigationConfig, {
          versioned: (config) => {
            config.versions.forEach((v) => {
              const indexSegmentRecords =
                generator.generateAlgoliaSearchRecordsForSpecificDocsVersion(
                  v.config,
                  {
                    type: "versioned",
              ...
              recordsWithoutIds.push(
                ...indexSegmentRecords.map(filterSearchRecord)
              );
            });
          },
          unversioned: (config) => {
...
describe("generateIndexSegmentsForDefinition", () => {
  const indexSegmentManager = new AlgoliaIndexSegmentManagerServiceImpl(
    createMockFdrApplication({})
  );
  it("should strip spaces from the version ID", () => {
    ...
    if (segments.type === "versioned") {
      expect(segments.configSegmentTuples[0]?.[1].id).toContain(
        "version-with-spaces"
      );
      expect(segments.configSegmentTuples[1]?.[1].id).toContain(
