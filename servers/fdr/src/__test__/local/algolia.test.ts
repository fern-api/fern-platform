...
describe("algolia index segment deleter", () => {
    it("correctly deletes old inactive index segments for unversioned docs", async () => {
        ...
        const inactiveOldIndexSegments = [
            createMockIndexSegment({ id: getUniqueSegment(), createdAt: subHours(oldSegmentCutoffDate, 4) }),
            createMockIndexSegment({ id: getUniqueSegment(), createdAt: subHours(oldSegmentCutoffDate, 3) }),
            createMockIndexSegment({ id: getUniqueSegment(), createdAt: subHours(oldSegmentCutoffDate, 2) }),
            createMockIndexSegment({ id: getUniqueSegment(), createdAt: subHours(oldSegmentCutoffDate, 1) }),
        ];
        const activeIndexSegments = [
            createMockIndexSegment({ id: getUniqueSegment(), createdAt: addHours(oldSegmentCutoffDate, 1) }),
        ];
