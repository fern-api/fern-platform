...
beforeEach(async () => {
  ...
  await fdrApplication.dao
    .generators()
    .deleteGenerators({ generatorIds: GENERATORS_FROM_OTHER_TESTS });
});
...
it("generator dao", async () => {
  ...
  const generatorUpdated = await fdrApplication.dao
    .generators()
    .listGenerators();
...
it("generator dao image non-unique", async () => {
  ...
  const generatorByImage = await fdrApplication.dao
    .generators()
    .getGeneratorByImage({ image: "my-cool/example" });
