...
it("generator version happy path update", async () => {
  ...
  const release = await fdrApplication.dao
    .generatorVersions()
    .getGeneratorRelease({
  ...
  const updatedRelease = await fdrApplication.dao
    .generatorVersions()
    .getGeneratorRelease({
      generator: FdrAPI.generators.GeneratorId("this-is-the-happy-path"),
      version: "3.1.2",
...
it("get generator that works for cli version", async () => {
  ...
  const releaseRetainMajor = await fdrApplication.dao
    .generatorVersions()
...
it("get generator retain major version", async () => {
  ...
  await fdrApplication.dao.generatorVersions().upsertGeneratorRelease({
    generatorRelease: {
      generatorId: FdrAPI.generators.GeneratorId(
  ...
  const releaseRetainMajor = await fdrApplication.dao
    ...
    .getLatestGeneratorRelease({
      getLatestGeneratorReleaseRequest: {
        generator: FdrAPI.generators.GeneratorId(
          "this-is-major-version-restricted"
        ),
        releaseTypes: ["GA"],
        generatorMajorVersion: 2,
