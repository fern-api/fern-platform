    ...
    expect(
        (
            await fdrApplication.dao
                .cliVersions()
                .getLatestCliRelease({ getLatestCliReleaseRequest: { release_types: [ReleaseType.Rc] } })
        )?.version,
    ).toEqual("1.1.0-rc.1");
    expect(
        (
            await fdrApplication.dao
                .cliVersions()
                .getLatestCliRelease({ getLatestCliReleaseRequest: { release_types: [ReleaseType.Ga] } })
        )?.version,
    ).toEqual("1.1.0");
});
