...
it("cli release with tags and URLs", async () => {
  ...
  const dbRelease = await fdrApplication.dao
    .cliVersions()
    .getCliRelease({ cliVersion: "0.1.2-rc13" });
  expect(dbRelease).not.toBeUndefined();
  expect(dbRelease?.tags).toEqual(release.tags);
  expect(dbRelease?.changelogEntry?.[0]?.links).toEqual(
    release.changelogEntry?.[0]?.links
  );
});
  ...
  expect(
    (
      await fdrApplication.dao
        .cliVersions()
        .getLatestCliRelease({ getLatestCliReleaseRequest: {} })
    )?.version
  ).toEqual("1.1.0");
  expect(
    (
...
it("cli version happy path update", async () => {
  ...
  expect(updatedRelease?.changelogEntry).toEqual(
    updateReleaseRequest.changelogEntry
  );
});
...
it("cli version rc versions", async () => {
  ...
  expect(noncifySemanticVersion("0.1.2-rc0")).toEqual(
    "00000-00001-00002-12-00000"
  );
  ...
  expect(noncifySemanticVersion("0.1.2-rc.1")).toEqual(
    "00000-00001-00002-12-00001"
  );
});
