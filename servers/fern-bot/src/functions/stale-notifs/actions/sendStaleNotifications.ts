...
import {
  PullRequest,
  PullRequestState,
} from "@fern-fern/paged-generators-sdk/api";
...
export async function sendStaleNotificationsInternal(env: Env): Promise<void> {
  ...
  for await (const pull of botPulls) {
    if (
      ...
      EXCLUDE_ORGS.has(pull.repositoryOwner)
    ) {
      continue;
    }

    if (pull.createdAt < new Date(Date.now() - STALE_IN_MS)) {
      orgPullMap.set(pull.repositoryOwner, [
  ...
  const upgradesSlackClient = new SlackService(
  ...
  for (const [org, pulls] of orgPullMap) {
    ...
    if (maybeApiSpecPull != null || versionUpdatePulls.length > 0) {
      const allPulls = versionUpdatePulls.concat(
        maybeApiSpecPull ? [maybeApiSpecPull] : []
      );
      ...
      upgradesSlackClient.notifyStaleUpgradePRs({
        ...
        retoolLink:
          "https://buildwithfern.retool.com/apps/703271ca-7777-11ef-aecd-ab097775918e/Stale%20Pulls",
  ...
  const teamPullsSlackClient = new SlackService(
    env.FERNIE_SLACK_APP_TOKEN,
    env.CUSTOMER_PULLS_SLACK_CHANNEL
  );
