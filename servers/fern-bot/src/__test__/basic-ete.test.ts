...
beforeEach(async () => {
  ...
  await app.eachRepository(async (installation) => {
    ...
    try {
      ...
      console.log(
        "Branches do not exist, continuing, consider `beforeEach` a success.",
        e
      );
    }
  });
...
it(
  ...
  async () => {
    ...
    await app.eachRepository(async (installation) => {
      ...
      const cliVersion = cleanStdout(
        (await execFernCli("--version", fullRepoPath)).stdout
      );
      ...
      try {
        ...
        const upgradedCliVersion = cleanStdout(
          (await execFernCli("--version", fullRepoPath)).stdout
        );
