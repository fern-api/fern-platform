...
<Callout intent="info">
  Once an invoice is in the NEW state, the amount, currency, dueDate, payerId and
  vendorId **cannot** be changed.
</Callout>
...
<Callout intent="info">
  If the entity does *NOT* have an [approval
  policy](/api-reference/entity/create-approval-policy) set, the invoice will
  automatically move into the APPROVED state and will not be in the NEW
  state.
</Callout>
...
<Callout intent="info">
  Approvals and Rejections create a [comment](/api-reference/invoice/comment/add-a-comment) on
  the invoice. You can use this to show a list of comments and approvals, as the
  comments will have the approver details as part of the response.
</Callout>
...
<Callout intent="info">
  If you are using custom payment rails, you are responsible for moving the
  invoice into this state once the funds have settled.
</Callout>