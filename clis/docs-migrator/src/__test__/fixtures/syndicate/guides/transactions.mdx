...
For complete webhook documentation, see our [feature post](/infrastructure/webhooks) and its corresponding [endpoints](/open-api/webhook/post-create-callback).

### Query the status of your transaction

You can query an individual transaction with the [Get Request by ID](/open-api/transactions/get-request) endpoint using the `transactionId` from the steps above:
