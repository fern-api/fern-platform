...
2. Create an `encrypted_key` by [sending an API request](../api-v1/post/accounts.mdx) to Bland.
...
2. Upload your numbers [through the API](../api-v1/post/inbound-insert.mdx).
...
Once you update an inbound number through the [Dev Portal](https://app.bland.ai) or [API](../api-v1/post/inbound-number-update.mdx), that number will be automatically configured to run on Bland's infrastructure. No additional steps are required!
