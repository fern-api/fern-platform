...
Here's how to make an [Inbound Agent](../api-v1/post/inbound-number-update.mdx) remember past conversations they've had with callers. There's a lot of other useful information we can integrate, like how long ago that call occurred and the call's duration.
...
By far, the easiest way to test out your `dynamic_data` configuration is via the [/dynamic_data/test](../api-reference/endpoint/dynamic_validate.mdx) endpoint. It returns the original configuration, every raw response, and the final variables after parsing is applied.
