...
<Steps>
  <Step title="Obtain your platform-wide access token">
    ...
    Use your Layer-provided `client_id` and `client_secret` to obtain a platform-wide access token. See [Authentication](../authentication.mdx) for more information on this process.
