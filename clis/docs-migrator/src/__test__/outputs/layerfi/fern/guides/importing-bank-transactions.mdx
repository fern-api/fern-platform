...
To import bank transactions to Layer, make a call to the [Import Bank Transactions](../api-reference/import-transactions.mdx) endpoint. This endpoint takes in the data on a bank transaction that is needed for Layer’s categorization and accounting logic.

Note that the `bank_account_id` field should correspond to the `internal_bank_account_ids` field specified when [onboarding the Business](business-onboarding.mdx) as in the example below.
...
The API will respond with a list of ingested [Bank Transactions](../api-reference/bank-transaction.mdx).
