...
To import invoices, make a call to the [Create Invoice](../api-reference/create-invoice-payments-from-batch.mdx) endpoint. This endpoint takes in the data on the sale your customer made, what was sold, any associated taxes, and how the sale was paid for.

### Invoices without payments
Invoices can be created without any payments associated with them. Payments towards the invoice can then be recorded later on using the [Record an Invoice Payment](../api-reference/create-invoice-payments-from-batch.mdx) endpoint.
...
The API will respond with an [Invoice](../api-reference/invoice.mdx) object.
