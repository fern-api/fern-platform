...
const EDGE_FLAGS = [
  ...
  "search-v2" as const,
  "playground-fab-enabled" as const,
];
...
export async function getEdgeFlags(domain: string): Promise<EdgeFlags> {
  try {
    ...
    const isPlaygroundFabEnabled = checkDomainMatchesCustomers(
      domain,
      config["playground-fab-enabled"]
    );
