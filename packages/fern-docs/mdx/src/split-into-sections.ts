...
export function splitMarkdownIntoSections(
  ...
  if (heading == null) {
    ...
    const textBeforeFirstHeading = markdownContent
  ...
  while (heading != null) {
    ...
    const textBeforeNextHeading = markdownContent
      .slice(heading.start + heading.length, nextHeading?.start)
      .trim();
    ...
    if (nextHeading != null) {
      ...
      if (nextHeading.depth > heading.depth) {
        ...
        while (
          stack.length > 0 &&
          (stack[stack.length - 1]?.depth ?? 0) >= nextHeading.depth
        ) {
          stack.pop();
        }
      }
