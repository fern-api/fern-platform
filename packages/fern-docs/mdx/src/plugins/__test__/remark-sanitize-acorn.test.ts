...
describe("remarkSanitizeAcorn", () => {
    ...
    it("should escape identifiers with multiple nested braces", () => {
        const result2 = sanitizeAcorns("# hi {foo {bar} {baz}}", [
            "foo",
            "baz",
        ]);
    ...
    it("should escape Promises", () => {
        ...
        const result2 = sanitizeAcorns(
            "# hi {Promise.resolve(1).then(() => {})}"
        );
        expect(result2).toBe("# hi \\{Promise.resolve(1).then(() => \\{})}\n");

        const result3 = sanitizeAcorns(
            "# hi {await new Promise((resolve) => setTimeout(resolve, 1))}"
        );
        expect(result3).toBe(
            "# hi \\{await new Promise((resolve) => setTimeout(resolve, 1))}\n"
        );

        const result4 = sanitizeAcorns(
            "# hi {new Promise((resolve) => setTimeout(resolve, 1))}"
        );
        expect(result4).toBe(
            "# hi \\{new Promise((resolve) => setTimeout(resolve, 1))}\n"
        );
    ...
    it("should escape identifiers inside JSX children", () => {
        const result2 = sanitizeAcorns("<div {...otherProps}>hi {foo}</div>", [
            "foo",
        ]);
    ...
    it("should not escape identifiers that had been used in JSX attributes", () => {
        ...
        const result5 = sanitizeAcorns(
            "<div style={{ color: foo }}>hi {foo + 1}</div>"
        );
