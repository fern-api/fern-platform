    ...
    return (tree) => {
        ...
        visit(tree, (node, index, parent) => {
            if (node.type === "mdxjsEsm") {
                const { identifiers, jsxIdentifiers, isAwaited } =
                    collectIdentifiers(node.data?.estree);
            ...
            if (
                ...
                node.attributes.forEach((attr) => {
                    if (
                        ...
                        const { identifiers, jsxIdentifiers } =
                            collectIdentifiers(attr.value.data?.estree);
                        identifiers.forEach((id) =>
                            allowedIdentifiersSet.add(id)
                        );
                        jsxIdentifiers.forEach((id) =>
                            allowedIdentifiersSet.add(id)
                        );
                    }
                });
