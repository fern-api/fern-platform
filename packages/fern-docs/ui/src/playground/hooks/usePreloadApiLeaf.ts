...
const fetcher = (url: string): Promise<ApiDefinition> =>
    fetch(url).then((res) => res.json());

export function usePreloadApiLeaf(): (
        ...
        useCallbackOne(async (get, set, node: NavigationNodeApiLeaf) => {
            const route = selectApiRoute(
                ...
                `/api/fern-docs/api-definition/${encodeURIComponent(node.apiDefinitionId)}/${visitDiscriminatedUnion(
                    ...
                    endpoint: (node) =>
                        `endpoint/${encodeURIComponent(node.endpointId)}`,
                    webSocket: (node) =>
                        `websocket/${encodeURIComponent(node.webSocketId)}`,
                    webhook: (node) =>
                        `webhook/${encodeURIComponent(node.webhookId)}`,
                })}`
            );
