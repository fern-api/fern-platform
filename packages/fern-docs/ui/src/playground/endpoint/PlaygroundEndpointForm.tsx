...
export const PlaygroundEndpointForm: FC<PlaygroundEndpointFormProps> = ({
    ...
    const setHeaders = useCallback(
        (value: ((old: unknown) => unknown) | unknown) => {
            setFormState((state) => ({
                ...state,
                headers:
                    typeof value === "function" ? value(state.headers) : value,
            }));
        },
        [setFormState]
    );
            ...
            setBody((old) => {
                return {
                    ...
                    value:
                        typeof value === "function"
                            ? value(
                                  old?.type === "json" ? old.value : undefined
                              )
                            : value,
                };
            });
    ...
    const setBodyOctetStream = useCallback(
        (
            value:
                | ((old: File | undefined) => File | undefined)
                | File
                | undefined
        ) => {
            setBody((old) => {
