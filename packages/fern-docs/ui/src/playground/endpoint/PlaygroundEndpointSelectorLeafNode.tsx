...
>(({ node, filterValue, active, closeDropdown }, ref) => {
            ...
            () =>
                atom((get) => {
                    const definition = get(
                        getApiDefinitionAtom(node.apiDefinitionId)
                    );
                    if (definition == null) {
                        return undefined;
                    }
                    if (node.type === "endpoint") {
                        return definition.endpoints[node.endpointId]
                            ?.description;
                    } else if (node.type === "webSocket") {
                        return definition.websockets[node.webSocketId]
                            ?.description;
                    }
                    return undefined;
                }),
    ...
    const tooltipContent =
        description != null ? (
            <Markdown mdx={description} size="xs" />
        ) : undefined;
