...
export class PlaygroundCodeSnippetResolver {
    ...
    public resolve(
        ...
        if (lang === "curl") {
            ...
            return (
                this.toTypescriptSdkSnippet(apiDefinition) ??
                this.toTypescriptFetch()
            );
        } else if (lang === "python") {
            return (
                this.toPythonSdkSnippet(apiDefinition) ??
                this.toPythonRequests()
            );
        } else {
    ...
    constructor(
        ...
        if (
            ...
            this.headers["Content-Type"] =
                this.context.endpoint.request.contentType;
        }
        ...
        if (
            ...
            if (this.context.endpoint.snippetTemplates.typescript != null) {
                this.typescriptSdkResolver = new SnippetTemplateResolver({
                    payload: convertToCustomSnippetPayload(
                        formState,
                        authState
                    ),
                    endpointSnippetTemplate: {
                        ...
                        endpointId: {
                            path: toCurlyBraceEndpointPathLiteral(
                                this.context.endpoint.path
                            ),
                            method: this.context.endpoint.method,
                            identifierOverride: undefined,
                        },
                        snippetTemplate:
                            this.context.endpoint.snippetTemplates.typescript,
                        apiDefinitionId: undefined,
                        additionalTemplates: undefined,
                    },
                    apiDefinitionGetter: async (id) => {
                        const response =
                            await provideRegistryService().api.v1.read.getApi(
            ...
            if (this.context.endpoint.snippetTemplates.python != null) {
                this.pythonRequestsResolver = new SnippetTemplateResolver({
                    ...
                    endpointSnippetTemplate: {
                        ...
                        snippetTemplate:
                            this.context.endpoint.snippetTemplates.python,
                        apiDefinitionId: undefined,
                        additionalTemplates: undefined,
                    },
                    apiDefinitionGetter: async (id) => {
    ...
    public toTypescriptFetch(): string {
        ...
        if (
            unknownToString(headers["Content-Type"]).includes(
                "multipart/form-data"
            )
        ) {
    ...
    public toTypescriptSdkSnippet(
        ...
        const resolvedTemplate =
            this.typescriptSdkResolver.resolve(apiDefinition);
    ...
    public toPythonSdkSnippet(
        ...
        const resolvedTemplate =
            this.pythonRequestsResolver.resolve(apiDefinition);
