...
export function matchesTypeReference(
    ...
    return visitDiscriminatedUnion(
        ...
        object: (object) => {
            ...
            unwrapObjectType(object, types).properties.forEach((property) =>
        ...
        discriminatedUnion: (discriminatedUnion) => {
            ...
            return discriminatedUnion.variants.some((variant) => {
                ...
                const propertyMap = new Map<string, ObjectProperty>();
                unwrapObjectType(variant, types).properties.forEach(
        ...
        undiscriminatedUnion: (undiscriminatedUnion) =>
            undiscriminatedUnion.variants.some((variant) =>
        ...
        list: (listType) =>
            Array.isArray(value) &&
            value.every((item) =>
                matchesTypeReference(listType.itemShape, item, types)
            ),
        set: (setType) =>
            Array.isArray(value) &&
            value.every((item) =>
                matchesTypeReference(setType.itemShape, item, types)
            ),
        map: (MapTypeContextProvider) =>
            isPlainObject(value) &&
            Object.keys(value).every((key) =>
                matchesTypeReference(
                    MapTypeContextProvider.valueShape,
                    value[key],
                    types
                )
            ),
