...
export function rehypeFernComponents(): (tree: Root) => void {
    return function (tree: Root): void {
        ...
        let request:
            | { path: string; method: string; example: string }
            | undefined;

        visit(tree, (node) => {
            if (isMdxJsxElementHast(node)) {
                const isRequestSnippet = node.name === "EndpointRequestSnippet";
                const isResponseSnippet =
                    node.name === "EndpointResponseSnippet";
        ...
        const visitor: Visitor = (node, index, parent) => {
            ...
            if (isMdxJsxElementHast(node) && node.name != null) {
                if (node.name === "Steps" || node.name === "StepGroup") {
                    ...
                    return transformAccordionGroup(
                        node,
                        index,
                        parent,
                        visitor
...
function transformTabItem(
    ...
    const child = {
        ...
        name: "TabGroup",
        attributes: [
            unknownToMdxJsxAttribute("tabs", tabs),
            ...node.attributes,
        ],
        children: [],
    };
