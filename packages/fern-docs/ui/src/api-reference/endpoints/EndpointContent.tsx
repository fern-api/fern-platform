...
const EndpointContentCodeSnippets = dynamic(
    () =>
        import("./EndpointContentCodeSnippets").then(
            (mod) => mod.EndpointContentCodeSnippets
        ),
    { ssr: true }
);
...
export const EndpointContent = memo<EndpointContent.Props>((props) => {
    ...
    const [hoveredRequestPropertyPath, setHoveredRequestPropertyPath] =
    ...
    const onHoverRequestProperty = useCallback(
        (
            ...
            setHoveredRequestPropertyPath(
                isHovering ? jsonPropertyPath : undefined
            );
        },
        [setHoveredRequestPropertyPath]
    );
        ...
        (statusCode: number | string | undefined) => {
            setSelectedExampleKey((prev) => {
                ...
                return {
                    ...prev,
                    statusCode:
                        statusCode != null ? String(statusCode) : undefined,
                    responseIndex: 0,
                };
            });
        },
                ...
                if (statusCodeOrName != null) {
                    const error = endpoint.errors?.find((e) =>
                        typeof statusCodeOrName === "number"
                            ...
                            : convertNameToAnchorPart(e.name) ===
                              statusCodeOrName
                    );
                    if (error != null) {
    ...
    const responseCodeSnippet = useMemo(
        () => JSON.stringify(responseJson, undefined, 2),
        [responseJson]
    );

    const selectedExampleClientLineCount =
        selectedExample?.code.split("\n").length ?? 0;

    const selectorHeight =
        Object.keys(examplesByKeyAndStatusCode).length > 1 ? GAP_6 + 24 : 0;
            ...
            () =>
                selectAtom(
                    atom((get): [number, number] => {
                        const isMobileSidebarEnabled = get(
                            MOBILE_SIDEBAR_ENABLED_ATOM
                        );
                        const contentHeight = get(CONTENT_HEIGHT_ATOM);
                        if (isMobileSidebarEnabled) {
                            const requestLines = Math.min(
                                MOBILE_MAX_LINES + 1,
                                selectedExampleClientLineCount
                            );
                            const responseLines = Math.min(
                                MOBILE_MAX_LINES + 1,
                                jsonLineLength
                            );
                            const requestContainerHeight =
                                requestLines * LINE_HEIGHT + CONTENT_PADDING;
                            const responseContainerHeight =
                                responseLines * LINE_HEIGHT + CONTENT_PADDING;
                            return [
                                requestContainerHeight,
                                responseContainerHeight,
                            ];
                        }
                        const maxRequestContainerHeight =
                            selectedExampleClientLineCount * LINE_HEIGHT +
                            CONTENT_PADDING;
                        const maxResponseContainerHeight =
                            jsonLineLength * LINE_HEIGHT + CONTENT_PADDING;
                        ...
                        if (
                            ...
                            const remainingContainerHeight =
                                containerHeight -
                                maxRequestContainerHeight -
                                GAP_6;
                            return [
    ...
    const initialExampleHeight =
        requestHeight +
        responseHeight +
        (responseHeight > 0 && requestHeight > 0 ? GAP_6 : 0) +
        padding;

    const [exampleHeightWithoutPadding, setExampleHeight] =
        useState(initialExampleHeight);
        ...
        >
            ...
            >
                <EndpointContentHeader
                    ...
                    streamToggle={props.streamToggle}
                />
