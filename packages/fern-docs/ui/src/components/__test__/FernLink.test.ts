...
describe("checkIsExternalUrl", () => {
    it("returns true for external URLs", () => {
        expect(checkIsExternalUrl(toUrlObject("https://example.com"))).toBe(
            true
        );
        expect(checkIsExternalUrl(toUrlObject("http://example.com"))).toBe(
            true
        );
        expect(
            checkIsExternalUrl(toUrlObject("mailto:hello@example.com"))
        ).toBe(true);
...
describe("checkIsRelativeUrl", () => {
    ...
    it("returns false for external URLs", () => {
        expect(checkIsRelativeUrl(toUrlObject("https://example.com"))).toBe(
            false
        );
        expect(checkIsRelativeUrl(toUrlObject("http://example.com"))).toBe(
            false
        );
        expect(
            checkIsRelativeUrl(toUrlObject("mailto:hello@example.com"))
        ).toBe(false);
...
describe("toUrlObject", () => {
    it("retains #hash and ?search", () => {
        expect(formatUrlString(toUrlObject("/path#hash"))).toEqual(
            "/path#hash"
        );
...
describe("resolveRelativeUrl", () => {
    ...
    it("resolves relative urls", () => {
        expect(resolveRelativeUrl("/base/path", "a/b/c")).toEqual(
            "/base/a/b/c"
        );
    ...
    it("drops #hash from the left side", () => {
        ...
        expect(
            resolveRelativeUrl("/base/path#hash", formatUrlString({}))
        ).toEqual("/base/path");
