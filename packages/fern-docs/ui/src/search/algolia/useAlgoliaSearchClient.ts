...
import {
    ...
    type LiteClient as SearchClient,
...
export function useAlgoliaSearchClient():
    ...
    return useMemo(() => {
        ...
        if (searchConfig.algolia.searchApiKey.type === "unversioned") {
            return [
                algoliasearch(
                    ...
                    searchConfig.algolia.searchApiKey.value
                ),
        ...
        if (searchConfig.algolia.searchApiKey.type === "versioned") {
            ...
            const searchApiKey =
                searchConfig.algolia.searchApiKey.values[currentVersionId];
            assertNonNullish(
                searchApiKey,
                `Inconsistent State: Did not receive index segment for version "${currentVersionId}". This may indicate a backend bug.`
            );
            return [
                algoliasearch(searchConfig.algolia.appId, searchApiKey),
                searchConfig.algolia.index,
            ];
        }
        return;
