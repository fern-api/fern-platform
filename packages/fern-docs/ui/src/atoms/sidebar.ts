    ...
    useAtomEffect(
        useCallbackOne((get, set) => {
            ...
            set(INTERNAL_EXPANDED_SIDEBAR_NODES_ATOM, (prev) => {
                const childToParentsMap = get.peek(
                    SIDEBAR_CHILD_TO_PARENTS_MAP_ATOM
                );
            ...
            () =>
                atom((get) => {
                    ...
                    return (
                        ...
                        implicitExpandedNodes.has(nodeId)
                    );
                }),
            [nodeId]
        )
        ...
        useMemoOne(
            () =>
                atom((get) => {
                    ...
                    const parentToChildrenMap = get(
                        SIDEBAR_PARENT_TO_CHILDREN_MAP_ATOM
                    );
                    return (
                        parentToChildrenMap
                            .get(parentId)
                            ?.includes(selectedNodeId) ?? false
                    );
                }),
            [parentId]
        )
    ...
    return useAtomCallback(
        useCallbackOne(
            (get, set) => {
                ...
                const childToParentsMap = get(
                    SIDEBAR_CHILD_TO_PARENTS_MAP_ATOM
                );

                set(INTERNAL_EXPANDED_SIDEBAR_NODES_ATOM, (prev) => {
                    const expandedNodes = new Set(prev.expandedNodes);
                    const implicitExpandedNodes = new Set(
                        prev.implicitExpandedNodes
                    );
...
export const DISMISSABLE_SIDEBAR_OPEN_ATOM = atom(
    (get) => {
        ...
        if (isMobileSidebarEnabled) {
            return (
                isMobileSidebarOpen || (isDesktopSidebarOpen && !isMobileScreen)
            );
        } else {
        ...
        useCallbackOne((get, set) => {
            ...
            const handleMessage = (event: MessageEvent) => {
                if (event.data === "openSearchDialog") {
                    ...
                    set(
                        THEME_ATOM,
                        get.peek(THEME_ATOM) === "dark" ? "light" : "dark"
                    );
