...
export const PLAYGROUND_NODE_ID = atom(
    (get) => {
        const playgroundParam =
            get(LOCATION_ATOM).searchParams?.get("playground");
        ...
        const node = nodes.slugMap.get(
            FernNavigation.slugjoin(playgroundParam)
        );
...
export const PLAYGROUND_AUTH_STATE_BEARER_TOKEN_ATOM = atom(
    (get) => ({
        token:
            ...
            get(FERN_USER_ATOM)?.playground?.initial_state?.auth
                ?.bearer_token ??
            "",
    }),
    (
...
export const PLAYGROUND_AUTH_STATE_BEARER_TOKEN_IS_RESETTABLE_ATOM = atom(
    (get) => {
        ...
        return (
            injectedBearerAuth != null && inputBearerAuth !== injectedBearerAuth
        );
    }
);
...
export const PLAYGROUND_AUTH_STATE_BASIC_AUTH_ATOM = atom(
    ...
    (
        ...
        set,
        update:
            | SetStateAction<Partial<PlaygroundAuthStateBasicAuth>>
            | typeof RESET
    ) => {
        ...
        useCallbackOne(
            async (get, set, node?: FernNavigation.NavigationNodeApiLeaf) => {
                ...
                if (node == null) {
                    ...
                    if (
                        currentNode != null &&
                        FernNavigation.isApiLeaf(currentNode)
                    ) {
