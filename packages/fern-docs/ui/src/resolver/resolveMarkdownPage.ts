            ...
            await Promise.all(
                [...apiDefinitionIds].map(
                    async (
                        ...
                        | [id: FernNavigation.ApiDefinitionId, ApiDefinition]
                        | undefined
                    > => {
                        ...
                        if (apiDefinition == null) {
                            // eslint-disable-next-line no-console
                            console.error(
                                `Failed to load API definition for ${id}`
                            );
                            return;
                        }
...
> {
    ...
    if (frontmatter["edit-this-page-url"] == null) {
        frontmatter["edit-this-page-url"] =
            markdownLoader.getEditThisPageUrl(node);
    }
