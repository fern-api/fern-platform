    ...
    <div className="explorer-endpoint">
      <div className="bg-tag-default flex h-10 min-w-0 flex-1 shrink items-center gap-2 rounded-lg px-4 py-2 max-sm:h-8 max-sm:px-2 max-sm:py-1 sm:rounded-[20px]">
        {method != null && (
          <HttpMethodBadge
            method={method}
            className="explorer-endpoint-method"
          />
        )}
        <span
          className={cn(
            environmentIds.length > 1
              ? "explorer-endpoint-url-with-switcher"
              : "explorer-endpoint-url",
            "flex w-full flex-row",
            "items-baseline"
          )}
        >
          <span className="explorer-endpoint-baseurl max-sm:hidden">
            <MaybeEnvironmentDropdown
              environmentId={environmentId}
              baseUrl={baseUrl}
              options={options}
              small
              urlTextStyle="explorer-endpoint-baseurl max-sm:hidden"
              protocolTextStyle="explorer-endpoint-baseurl max-sm:hidden"
              editable
              ...
              pathParameter: (pathParameter) => {
                ...
                return (
                  <span
                    key={idx}
                    className={cn({
                      "t-accent bg-accent-highlight rounded px-1 before:content-[':']":
                        stateValue.length === 0,
        ...
        <CopyToClipboardButton
          className="explorer-endpoint-copy-button"
          content={() =>
            buildRequestUrl({
              path,
