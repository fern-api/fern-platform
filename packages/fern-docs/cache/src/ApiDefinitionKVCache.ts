...
export class ApiDefinitionKVCache {
    private static instance: Map<string, ApiDefinitionKVCache> = new Map<
        string,
        ApiDefinitionKVCache
    >();
    public static getInstance(
        domain: string,
        api: ApiDefinition.ApiDefinitionId
    ): ApiDefinitionKVCache {
    ...
    private async setResolvedDescription(
        ...
        await this.markdownCache.setMarkdownText(
            `${this.api}:${key}`,
            description
        );
    }
    ...
    public async batchResolveDescriptions(
        ...
        const misses = Object.entries(records).filter(
            ([key]) => !(key in hits)
        );
