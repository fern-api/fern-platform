...
<ParamField path="children" type="string | JSX" required={true}>
  The content to be displayed when the tab is selected. Can include text,
...
<Tabs>
  ...
  <Tab title="[Third Tab](https://github.com)">
    💪 Here's content that's only inside the third Tab. The title of this tab is
    a link.
  </Tab>
</Tabs>
