      ...
      {tabs.map((tab) => (
        <Tabs.TabsTrigger key={tab.id} value={tab.id} asChild>
          <FernLink
            className={cn(
              "min-h-8 lg:min-h-9",
              "hover:t-accent group flex min-w-0 flex-1 select-none items-center justify-start rounded-lg py-2 text-base lg:px-3 lg:text-sm",
              "data-[state=inactive]:text-muted data-[state=active]:text-accent-a11 [&_svg]:size-4",
          ...
          >
            <span
              className={cn(
                "bg-card-surface border-default mr-4 flex size-6 items-center justify-center rounded-md border shadow-sm",
