...
export function groupExamplesByLanguageKeyAndStatusCode(
  ...
  function addCodeExample(
    ...
    const existing = (((toRet[key.language] ??= {})[key.exampleKey] ??= {})[
      key.statusCode
    ] ??= []);
    if (
      existing.some((e) =>
        isEqual(
          e.exampleCall.responseBody,
          codeExample.exampleCall.responseBody
        )
      )
    ) {
      return;
...
export function selectExampleToRender(
  ...
  const examplesByStatusCode =
    ...
    examplesByKeyAndStatusCode[
      Object.keys(examplesByKeyAndStatusCode)[0] ?? ""
    ] ??
    {};
  ...
  let selectedExample =
    examplesByStatusCode[statusCode ?? ""]?.[responseIndex ?? 0];

  // if the status code is not found, we should attempt to find a different example that has the same status code
  if (statusCode != null) {
    selectedExample ??= Object.values(examplesByKeyAndStatusCode).find(
      (examplesByStatusCode) => {
  ...
  return {
    ...
    selectedExample,
    examplesByStatusCode:
      reverseLookup?.examplesByStatusCode ?? examplesByStatusCode,
    examplesByKeyAndStatusCode:
      reverseLookup?.examplesByKeyAndStatusCode ?? examplesByKeyAndStatusCode,
  };
}
...
export function reverseLookupSelectedExample(
  ...
  const examplesByKeyAndStatusCode =
    examplesByLanguageKeyAndStatusCode[selectedExample.language] ?? {};
  const examplesByStatusCode =
    examplesByKeyAndStatusCode[selectedExample.exampleKey] ?? {};
