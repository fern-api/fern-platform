...
export function convertToCustomSnippetPayload(
  ...
  return {
    pathParameters: Object.entries(formState.pathParameters).map(
      ([name, value]) => ({ name, value })
    ),
    queryParameters: Object.entries(formState.queryParameters).map(
      ([name, value]) => ({ name, value })
    ),
...
function toAuthPayload(
  authState: PlaygroundAuthState
): Snippets.AuthPayload | undefined {
  if (authState.oauth) {
    return {
      type: "bearer",
      token:
        authState.oauth.userSuppliedAccessToken || authState.oauth.accessToken,
    };
