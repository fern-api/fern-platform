...
describe("getAuthState", () => {
    ...
    it("should handle oauth with ory", async () => {
        ...
        const fernToken = await signFernJWT(
            {},
            { secret: TEST_JWT_SECRET, issuer: ISSUER }
        );
    ...
    it("should handle sso with workos", async () => {
        ...
        if (!authStateBadToken.authed) {
            ...
            expect(authorizationUrl.searchParams.get("organization")).toBe(
                WORKOS_AUTH_CONFIG.organization
            );
            expect(authorizationUrl.searchParams.get("redirect_uri")).toBe(
        ...
        const getSessionFromTokenMock = vi.spyOn(
            session,
            "getSessionFromToken"
        );
        const toSessionUserInfoMock = vi
            .spyOn(session, "toSessionUserInfo")
            .mockImplementation(() =>
                Promise.resolve({
                    user: WORKOS_USER,
                    sessionId: "test",
