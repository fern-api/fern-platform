...
const getApi = async (domain: string, id: string) => {
  ...
  if (v1 == null) {
    console.debug(`Could not find definition for API: ${id}`);
    notFound();
  }
...
const getEndpointById = async (
  ...
  if (endpoint == null) {
    console.debug(`Could not find endpoint: ${endpointId}`);
    notFound();
  }
...
const getEndpointByLocator = async (
  ...
  console.debug(`Could not find endpoint ${method} ${path} in any API`);
  notFound();
};
...
const unsafe_getFullRoot = async (domain: string) => {
  ...
  if (root == null) {
    console.debug(`Could not find root node for ${domain}`);
    notFound();
  }
  return root;
};
...
const getNavigationNode = cache(
  async (
    ...
    if (node == null) {
      console.debug(`Could not find navigation node ${id}`);
      notFound();
    }
    return node;
  }
...
const getPage = cache(async (domain: string, pageId: string) => {
  ...
  if (page == null) {
    console.debug(`Could not find page definition for ${domain}:${pageId}`);
    notFound();
  }
...
const getLayout = cache(async (domain: string) => {
  ...
  if (!config) {
    console.debug(`Could not find configuration for ${domain}`);
    notFound();
  }
