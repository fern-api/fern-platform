...
export async function checkReleaseBlockers(
  releaseBlockersFilename: string
): Promise<void> {
  ...
  if (releaseBlockers == null) {
    console.error(
      chalk.red(`key 'release-blockers' is missing in ${filepath}`)
    );
    process.exit(1);
  }
  ...
  if (!Array.isArray(releaseBlockers)) {
    console.error(
      chalk.red(`'release-blockers' is not an array in ${filepath}`)
    );
    process.exit(1);
  }
  ...
  if (releaseBlockers.length > 0) {
    console.error(
      chalk.red(`Cannot release because there are blockers in ${filepath}`)
    );
    process.exit(1);
  }
}
