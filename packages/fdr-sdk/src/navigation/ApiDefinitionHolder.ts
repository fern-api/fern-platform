...
export class ApiDefinitionHolder {
    ...
    private constructor(public readonly api: APIV1Read.ApiDefinition) {
        [api.rootPackage, ...Object.values(api.subpackages)].forEach(
            (package_) => {
                const subpackageId = isSubpackage(package_)
                    ? package_.subpackageId
                    : ROOT_PACKAGE_ID;
                package_.endpoints.forEach((endpoint) => {
                    this.#endpoints.set(
                        ApiDefinitionHolder.createEndpointId(
                            endpoint,
                            subpackageId
                        ),
                        endpoint
                ...
                package_.webhooks.forEach((webhook) => {
                    this.#webhooks.set(
                        ApiDefinitionHolder.createWebhookId(
                            webhook,
                            subpackageId
                        ),
                        webhook
    ...
    public static createEndpointId(
        ...
        return FernNavigation.EndpointId(
