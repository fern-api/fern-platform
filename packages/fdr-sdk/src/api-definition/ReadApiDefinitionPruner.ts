...
import { ReadApiDefinitionHolder } from "./ReadApiDefinitionHolder";
import { ReadApiTypeIdVisitor } from "./ReadApiTypeIdVisitor";
...
export class ApiDefinitionPruner {
    ...
    private pruneTypes(
        ...
        globalHeaders?.forEach((header) => {
            ReadApiTypeIdVisitor.visitTypeReference(header.type, (typeId) => typeIds.add(typeId));
        });
    ...
    private expandTypeIds(typeIds: Set<APIV1Read.TypeId>): Set<APIV1Read.TypeId> {
        ...
        while (queue.length > 0) {
            ...
            if (typeId != null && !visitedTypeIds.has(typeId)) {
                ...
                const type = this.api.types[typeId];
                if (type) {
                    ReadApiTypeIdVisitor.visitTypeDefinition(type, (nestedTypeId) => {
    ...
    private prunePackage<T extends APIV1Read.ApiDefinitionPackage>(
        ...
        endpoints.forEach((endpoint) => {
            ReadApiTypeIdVisitor.visitEndpointDefinition(endpoint, (typeId) => typeIds.add(typeId));
        });
        websockets.forEach((websocket) => {
            ReadApiTypeIdVisitor.visitWebSocketChannel(websocket, (typeId) => typeIds.add(typeId));
        });
        webhooks.forEach((webhook) => {
            ReadApiTypeIdVisitor.visitWebhookDefinition(webhook, (typeId) => typeIds.add(typeId));
        });
    ...
    private pruneEndpoints(
        ...
        const found = endpoints.find(
            (endpoint) => ReadApiDefinitionHolder.createEndpointId(endpoint, subpackageId) === endpointId,
        );
    ...
    private pruneWebSockets(
        ...
        const found = websockets.find(
            (websocket) => ReadApiDefinitionHolder.createWebSocketId(websocket, subpackageId) === websocketId,
        );
    ...
    private pruneWebhooks(
        ...
        const found = webhooks.find(
            (webhook) => ReadApiDefinitionHolder.createWebhookId(webhook, subpackageId) === webhookId,
        );
