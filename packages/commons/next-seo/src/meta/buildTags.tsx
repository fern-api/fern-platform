...
const buildOpenGraphMediaTags = (
    ...
    return media.reduce((tags, medium, index) => {
        tags.push(<meta key={`og:${mediaType}:0${index}`} property={`og:${mediaType}`} content={medium.url} />);
...
export const buildTags = (config: NextSeoProps): ReactNode[] => {
    ...
    if (config.themeColor) {
        tagsToRender.push(<meta key="theme-color" name="theme-color" content={config.themeColor} />);
    }
    ...
    if (config.facebook) {
        if (config.facebook.appId) {
            tagsToRender.push(<meta key="fb:app_id" property="fb:app_id" content={config.facebook.appId} />);
        }
    }
    ...
    if (config.openGraph) {
            ...
            if (type === "profile" && config.openGraph.profile) {
                ...
                if (config.openGraph.book.tags && config.openGraph.book.tags.length) {
                    config.openGraph.book.tags.forEach((tag, index) => {
                        tagsToRender.push(<meta key={`book:tag:0${index}`} property="book:tag" content={tag} />);
                    });
                }
                ...
                if (config.openGraph.article.tags && config.openGraph.article.tags.length) {
                    config.openGraph.article.tags.forEach((tag, index) => {
                        tagsToRender.push(<meta key={`article:tag:0${index}`} property="article:tag" content={tag} />);
                    });
                }
            } else if (
                (type === "video.movie" ||
                ...
                if (config.openGraph.video.actors && config.openGraph.video.actors.length) {
                    config.openGraph.video.actors.forEach((actor, index) => {
                        if (actor.profile) {
                            tagsToRender.push(
                                <meta key={`video:actor:0${index}`} property="video:actor" content={actor.profile} />,
                            );
                        }
                ...
                if (config.openGraph.video.directors && config.openGraph.video.directors.length) {
                    config.openGraph.video.directors.forEach((director, index) => {
                        tagsToRender.push(
                            <meta key={`video:director:0${index}`} property="video:director" content={director} />,
                        );
                    });
                }
                ...
                if (config.openGraph.video.writers && config.openGraph.video.writers.length) {
                    config.openGraph.video.writers.forEach((writer, index) => {
                        tagsToRender.push(
                            <meta key={`video:writer:0${index}`} property="video:writer" content={writer} />,
                        );
                    });
                }
                ...
                if (config.openGraph.video.tags && config.openGraph.video.tags.length) {
                    config.openGraph.video.tags.forEach((tag, index) => {
                        tagsToRender.push(<meta key={`video:tag:0${index}`} property="video:tag" content={tag} />);
                    });
                }

                if (config.openGraph.video.series) {
                    tagsToRender.push(
                        <meta key="video:series" property="video:series" content={config.openGraph.video.series} />,
                    );
                }
            }
        }
        ...
        if (config.openGraph.siteName) {
            tagsToRender.push(<meta key="og:site_name" property="og:site_name" content={config.openGraph.siteName} />);
        }
    }
    ...
    if (config.additionalMetaTags && config.additionalMetaTags.length > 0) {
        config.additionalMetaTags.forEach(({ keyOverride, ...tag }) => {
            tagsToRender.push(
                <meta key={`meta:${keyOverride ?? tag.name ?? tag.property ?? tag.httpEquiv}`} {...tag} />,
            );
        });
    }
    ...
    if (config.additionalLinkTags?.length) {
        config.additionalLinkTags.forEach((tag) => {
            ...
            tagsToRender.push(
                <link key={`link${rest.keyOverride ?? rest.href}${rest.rel}`} {...rest} crossOrigin={crossOrigin} />,
            );
        });
    }
