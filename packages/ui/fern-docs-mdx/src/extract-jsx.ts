...
export function extractJsx(tree: HastRoot | MdastRoot): {
    ...
    visit(tree, (node) => {
        ...
        if (
            (node.type === "mdxFlowExpression" || node.type === "mdxTextExpression" || node.type === "mdxjsEsm") &&
            node.data &&
            node.data.estree
        ) {
    ...
    return {
        jsxElements: Array.from(jsxElements)
            ...
            .filter((name) => name !== "Fragment" && name.match(/^[A-Z][a-zA-Z0-9]*$/)),

        esmElements: Array.from(esmElements).filter((name) => name.match(/^[A-Z][a-zA-Z0-9]*$/)),
    };
}
