...
import { FilterCondition, QueryResults, Turbopuffer } from "@turbopuffer/turbopuffer";
...
export async function queryTurbopuffer(
    ...
    const authFilter: FilterCondition = authed
        ? ([
              ...
              [
                  ...
                  ...createPermutations(roles.filter((r) => r !== EVERYONE_ROLE)).map((perms) =>
                      createRoleFacet(perms),
                  ),
              ],
          ] as const)
        : ["authed", "NotEq", true];
    ...
    return reciprocalRankFusion(semanticResults, bm25Results) as unknown as FernTurbopufferRecord[];
}
...
function reciprocalRankFusion(bm25: ResultItem[], vector: ResultItem[], k: number = 60): ResultItem[] {
