...
export async function createMarkdownRecords({
    ...
    markdown,
    splitText,
}: CreateMarkdownRecordsOptions): Promise<FernTurbopufferRecordWithoutVector[]> {
    ...
    const base_root_markdown_record: BaseRecord = {
        ...
        attributes: {
            ...
            code_snippet_langs: code_snippet_langs.length > 0 ? code_snippet_langs : undefined,
        },
    };
    ...
    const subheadingRecords = flatten(
        await Promise.all(
            sections.map(async (section, i) => {
                ...
                const hierarchy: Record<"h0" | "h1" | "h2" | "h3" | "h4" | "h5" | "h6", string | undefined> = {
                    h0: title,
                ...
                const code_snippets = flatten(
                    compact([base.attributes.code_snippets, prepared.code_snippets?.map((c) => c.code)]),
                );

                const code_snippet_langs = flatten(
                    compact([base.attributes.code_snippet_langs, prepared.code_snippets?.map((c) => c.lang ?? "")]),
                );

                const chunked_content = flatten(
                    await Promise.all([splitText(markdownContent), ...code_snippets.map((code) => splitText(code))]),
                );
