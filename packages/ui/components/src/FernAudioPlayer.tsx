    ...
    return (
        <div className={clsx("fern-audio-player", props.className)}>
            <audio
                ...
                className="hidden"
                onLoadedMetadata={(event: React.SyntheticEvent<HTMLAudioElement, Event>) => {
            ...
            <div className="relative flex flex-1 flex-col justify-center gap-1">
                <p className="m-0 inline-flex justify-center gap-2 text-center">
                    ...
                    {props.author && (
                        <>
                            <span>&mdash;</span>
                            <span className="t-muted max-w-[20vw] truncate">{props.author}</span>
                        </>
                    )}
                </p>
                ...
                <div className="mr-1 flex h-full items-center justify-center">
                    <FernButtonGroup className="items-center">
                        ...
                        />
                        <FernButton
                            icon={isPlaying ? <Pause /> : <Play className="fill-accent-contrast -mr-0.5" />}
                            variant="filled"
                            intent="primary"
                            rounded
                            size="large"
...
function FernAudioProgress({ duration, audioRef }: { duration: number; audioRef: React.RefObject<HTMLAudioElement> }) {
