...
import { renderReferencedTypePreviewPart } from "./ReferencedTypePreviewPart";
...
export function renderTypeShorthand(
    ...
    return visitDiscriminatedUnion(type, "type")._visit<ReactElement | string>({
        id: ({ value: typeId }) => renderReferencedTypePreviewPart(typeId, plural, withArticle, resolveTypeById),
...
export const TypeShorthand: React.FC<TypeShorthand.Props> = ({
    ...
    const { resolveTypeById: fallback } = useApiDefinitionContext();
    return <>{renderTypeShorthand(type, plural, withArticle, resolveTypeById ?? fallback)}</>;
};
