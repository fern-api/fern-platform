...
export function useActiveValueListeners<T extends string>(initialValue: T | undefined): Return<T> {
    ...
    const listeners = useRef<Record<T, (() => void)[]>>({} as Record<T, (() => void)[]>);
        ...
        return () => {
            ...
            if (listenersForValue != null) {
                ...
                if (indexOfListenerToDelete === -1) {
                    ...
                    console.warn("Failed to deregister listener for useActiveValueListeners.");
                } else {
