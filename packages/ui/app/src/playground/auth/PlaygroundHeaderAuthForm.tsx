...
function headerAtom(headerName: string) {
    return atom(
        (get) => get(PLAYGROUND_AUTH_STATE_HEADER_ATOM).headers[pascalCaseHeaderKey(headerName)],
        (_get, set, change: SetStateAction<string> | typeof RESET) => {
            set(PLAYGROUND_AUTH_STATE_HEADER_ATOM, ({ headers }) => {
                const nextHeaderValue =
                    typeof change === "function" ? change(headers[pascalCaseHeaderKey(headerName)] ?? "") : change;
                if (nextHeaderValue === RESET) {
                    return {
                        // note: this will remove all undefined values from the object
                        headers: JSON.parse(
                            JSON.stringify({ ...headers, [pascalCaseHeaderKey(headerName)]: undefined }),
                        ),
                    };
                }
                return {
...
function isHeaderResettableAtom(headerName: string) {
    return atom((get) => {
        const inputHeader = get(PLAYGROUND_AUTH_STATE_HEADER_ATOM).headers[pascalCaseHeaderKey(headerName)];
        const injectedHeader =
            get(FERN_USER_ATOM)?.playground?.initial_state?.headers?.[pascalCaseHeaderKey(headerName)];
