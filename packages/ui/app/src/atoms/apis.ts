...
export const getApiDefinitionAtom = atomFamily((apiDefinitionId: ApiDefinition.ApiDefinitionId) =>
    atom(
        (get) => get(SETTABLE_APIS_ATOM)[apiDefinitionId],
        (_get, set, apiDefinition: ApiDefinition.ApiDefinition) => {
            const merge = (prev: ApiDefinition.ApiDefinition | undefined) => {
                if (prev == null) {
                    return apiDefinition;
                } else {
                    return ApiDefinition.join(prev, apiDefinition);
                }
            };
