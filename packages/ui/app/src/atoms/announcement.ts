...
export const ANNOUNCEMENT_CONFIG_ATOM = atom((get) => get(DOCS_ATOM).announcement);

export const ANNOUNCEMENT_DISMISSED_STORAGE_ATOM = atomWithStorage("fern-announcement-dismissed", "");
...
export const ANNOUNCEMENT_DISMISSED_ATOM = atom(
    (get) => {
        ...
        return get(ANNOUNCEMENT_DISMISSED_STORAGE_ATOM) === announcementConfig.text;
    },
    (get, set, dismissed: boolean) => {
        if (dismissed) {
            set(ANNOUNCEMENT_DISMISSED_STORAGE_ATOM, get(ANNOUNCEMENT_CONFIG_ATOM)?.text ?? "");
        } else {
...
export const ANNOUNCEMENT_HEIGHT_ATOM = atom(
    (get) =>
        get(ANNOUNCEMENT_DISMISSED_ATOM) && !get(ANNOUNCEMENT_IS_DISMISSING_ATOM)
            ? 0
