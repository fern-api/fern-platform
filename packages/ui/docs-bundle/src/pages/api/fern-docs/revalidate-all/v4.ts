...
const handler: NextApiHandler = async (
    ...
    if (authConfig?.type === "basic_token_verification") {
        if (authConfig.allowlist == null || authConfig.allowlist.length === 0) {
            return res.status(200).json({ total: 0, results: [] });
        } else {
            allowlist = authConfig.allowlist;
        }
    }
    ...
    let batch = slugs.slice(start, start + limit);

    // if the allowlist is nonempty, it means some of the routes are protected by basic_token_verification
    if (allowlist.length > 0) {
        batch = batch.filter((slug) => withBasicTokenViewAllowed(allowlist, `/${slug}`));
    }
