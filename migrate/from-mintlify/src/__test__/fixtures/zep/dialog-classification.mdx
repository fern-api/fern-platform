...
<Tabs>
    ...
    <Tab title="Typescript">
        ```typescript
        const classes = ["class1", "class2", ..., "other"]
...
<Tabs group="memory">
    ...
    <Tab title="Typescript">
        ```typescript
        const classes = ["advanced", "beginner", ..., "dojo"]
        const classification = await client.memory.classifySession(
            session_id,
            "experience level",
            classes,
            "Classify the user's experience level. You may classify experience as 'dojo' only if the user is a black belt."
        )
        ```
    </Tab>
</Tabs>
