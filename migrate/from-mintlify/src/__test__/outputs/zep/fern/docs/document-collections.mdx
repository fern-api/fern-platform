...
*You can also use `ZepVectoreStore` as a retriever with langchain*
...
> *Please check the complete typescript [rag vector store chain example](https://github.com/getzep/zep-js/blob/main/examples/langchain/vector_store_example.ts)*
...
await client.document.updateCollection(
    collectionName,
    { description: "Charles Babbage's Babbage's Calculating Engine 2", metadata: { newfoo: "newbar" } }
);
```
